...
namespace PokeAPI
{
        ...
        where T : ApiObject<T>
    {
        ...
        public static T Create(JsonData source, T ret)
        {
            ...
            try
            {
                if (!ret.OverrideDefaultParsing)
                {
                    ...
                    ret.ResourceUri = new Uri(BASE_ADDR + source[RESU].ToString());
        ...
        protected static ApiResource ParseResource(JsonData data) => new ApiResource(data[NAME].ToString().Replace('-', ' '), new Uri(BASE_ADDR + data[RESU].ToString()));
